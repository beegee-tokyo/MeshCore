...
/** Receiver key prefix \todo implement API to setup this key */
uint8_t g_sensor_hub_pub_key_prefix[6];
/** Raw data packet in Cayenne LPP format */
WisCayenne g_solution_data(255);
/** Flag if payload transmission was successfull */
bool g_packet_sent = false;
/** Flag for Mesh loop to send sensor data as raw payload */
bool g_sensor_ready = false;

#if defined(NRF52_PLATFORM)
/** Timer to send sensor data frequentyl */
...
public:
	...
	void handleCmdFrame(size_t len)
	{
		...
		{ // send GroupChannel msg
			...
			if (txt_type == TXT_TYPE_PLAIN && sendGroupMessage(msg_timestamp, *_public, _prefs.node_name, text, len - i))
			{ // hard-coded to 'public' channel for now
				writeOKFrame();
			}
			else
			{
	...
	void loop()
	{
		...
		if (g_sensor_ready)
		{
			// _serial->writeFrame((const uint8_t *)"mesh::loop", 10);
			...
			// \todo make this flexible
			ContactInfo *recipient = searchContactsByPrefix((const char *)"Gateway");
			int8_t path_len = recipient->out_path_len;
			uint8_t *path = recipient->out_path;
			// Add this nodes pub_key to the payload
			g_solution_data.addMeshCoreID(LPP_MESHCORE_ID, self_id.pub_key);
			auto pkt = createRawData(g_solution_data.getBuffer(), g_solution_data.getSize());
			if ((recipient != NULL) && (pkt != NULL))
			{
				sendDirect(pkt, path, path_len);
				// sendFlood(pkt); // Hopefully possible in the future
				g_packet_sent = true;
			}
			else
			{
...
#error "need to define filesystem"
	...
	if (init_bme())
	{
		// sensor_timer.begin(1 * 60 * 1000, sensor_handler);
		sensor_timer.begin(30 * 1000, sensor_handler);
		sensor_timer.start();
	}
}
...
{
	// Serial.println("sensor_handler");
	g_solution_data.reset();
	if (read_bme())
	{
		uint16_t battery_millivolts = board.getBattMilliVolts();
		g_solution_data.addVoltage(LPP_CHANNEL_BATT, battery_millivolts/1000.0);
		Serial.flush();
		g_sensor_ready = true;
	}
}
